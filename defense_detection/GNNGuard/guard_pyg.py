...
import sys
sys.path.append('../../')
from gnn_model.gcn import GraphConvolution as GCNConv
...
class GCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, drop=False, weight_decay=5e-4, n_edge=1,with_relu=True,
                 with_bias=True, device=None, attention=None):
    ...
    def forward(self, x, adj):
        ...
        if self.attention:  # if attention=True, use attention mechanism
            ...
            edge_index = adj_2._indices()
            adj_values = adj_2._values()
        else:
            edge_index = adj._indices()
    ...
    def att_coef(self, fea, edge_index, is_lil=False, i=0):
        ...
        shape = (n_node, n_node)
        new_adj = torch.sparse.FloatTensor(att_adj, att_edge_weight, shape).to(fea.device)
        return new_adj
