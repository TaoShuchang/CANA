...
import sys
sys.path.append('../')
from modules.HAO import compute_homo_loss
sys.path.append('../../')
from utils import *
...
# pgd feature upd
def update_features(attacker, model, new_adj_tensor, features, inj_features, origin_labels, sec, target_idx, n_epoch=1999):
    ...
    inj_feat = torch.from_numpy(inj_features.astype('double')).float().to(attacker.device)
    model.eval()


    ori_label = origin_labels[target_idx]
    best_wrong_label = sec[target_idx]
    print('n_epoch',n_epoch)
    for i in range(n_epoch):
        inj_feat.requires_grad_(True)
        inj_feat.retain_grad()
        new_feat = torch.cat((features, inj_feat), dim=0)
        pred = model(new_feat, normalize_tensor(new_adj_tensor))
        # stablize the pred_loss, only if disguise_coe > 0
        atk_loss = F.relu(pred[target_idx, ori_label] - pred[target_idx, best_wrong_label])
        atk_loss = (atk_loss).mean()

        homo_loss = compute_homo_loss(new_adj_tensor, new_feat, inj_feat, n_total)
        # 进行梯度上升， max loss
        loss = - atk_loss - disguise_coe * homo_loss
        model.zero_grad()
        loss.backward()
        grad = inj_feat.grad.data
        # 进行梯度上升， max loss
        inj_feat = inj_feat.clone() + epsilon * grad.sign()
        ...
        if attacker.early_stop:
            ...
            if attacker.early_stop.stop:
                ...
                attacker.early_stop.reset()
                return inj_feat
        if attacker.verbose and i%100==0:
            print(
                "Attacking: Epoch {}, Loss: {:.5f}, Surrogate test score: {:.5f}, feat_min: {:.3f},feat_max: {:.3f}".format(i, loss, test_score, inj_feat.min(), inj_feat.max()),
