...
def main(args):
    ...
    if dataset == 'reddit':
        ...
        feat_init_max = feat_lim_max
        feat_init_min = feat_lim_min
    ...
    for epoch in range(args.st_epoch, args.nepochs):
        ...
        netD.eval()
        if dataset == 'ogbarxiv':
            inj_feat, loss_G_atk, loss_G_fake, train_loss_G, matk, acc_fake_G = update_features(attacker, surro_net, new_adj_tensor, feat, inj_feat, labels, sec, index_test, netD=netD, rep_net=rep_net, loss_type=args.loss_type, alpha=args.alpha, beta=args.beta, batch_loader_G=batch_loader_G, iter_loader_G=iter_loader_G)
        else:
            inj_feat, loss_G_atk, loss_G_fake, train_loss_G, matk, acc_fake_G = update_features(attacker, surro_net, new_adj_tensor, feat, inj_feat, labels, sec, index_test, netD=netD, rep_net=rep_net, loss_type=args.loss_type, alpha=args.alpha, beta=args.beta, fake_batch=fake_batch)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--nepochs', type=int, default=101, help='number of epochs')
